...
export function buildPortfolioWorkbook(portfolio: PortfolioRow[], transactions: BrokerTransaction[]): Uint8Array {
  ...
  const summaryRows: unknown[][] = [["ETF-Steuerassistent Österreich – Export", "Wert"], ["Exportiert am", new Date().toISOString()], ["Gespeicherte ETF-Jahre", portfolio.length], ["Importierte Transaktionen", transactions.length], ["", ""], ["Steuerjahr", "KZ 898", "KZ 937", "KZ 994", "KZ 892", "KZ 998", "Steuerpflichtige Basis", "Steuerschätzung"]];
  ...
  const txHeaders = ["Datum", "Art", "ISIN", "ETF/Wertpapier", "Stück (+ Kauf / − Verkauf)", "Stück absolut", "Broker", "Betrag", "Währung", "PDF-Datei", "Erkennung", "Importiert am"];
  const txRows = transactions.slice().sort((a, b) => a.date.localeCompare(b.date)).map((item) => [item.date, item.type === "buy" ? "Kauf" : "Verkauf", item.isin, item.securityName ?? "", item.type === "buy" ? item.units : -item.units, item.units, item.broker ?? "", item.amount ?? "", item.currency ?? "", item.sourceFile, item.confidence === "high" ? "hoch" : item.confidence === "medium" ? "mittel" : "niedrig", item.importedAt]);
  const files: Record<string, Uint8Array> = {
    ...
    "xl/worksheets/sheet3.xml": strToU8(sheet([txHeaders, ...txRows], [14,12,18,28,22,16,20,16,12,28,14,24], [7], [4,5])),
  };
